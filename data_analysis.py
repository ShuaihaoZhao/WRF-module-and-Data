...
plt.figure(figsize=(20,10))
plt.plot(forecasted_data.iloc[:,0],forecasted_data.iloc[:,4],label='forecasted')
...
for index,rows in part_real.iterrows():
    ...
    error_2=error_2+2*abs((forecasted_data.iloc[index,4])-(part_real.iloc[index,2]))/((part_real.iloc[index,2])+(forecasted_data.iloc[index,4]))
    counter+=1
